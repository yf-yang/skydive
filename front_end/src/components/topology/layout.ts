...
export class TopologyLayout {
    ...
    constructor(vm: TopologyComponent, selector: string) {
        ...
        let linksG = this.view.append('g').attr('class', 'links');
        this.link = linksG.selectAll<Link>('.link');

        let nodesG = this.view.append('g').attr('class', 'nodes');
    ...
    LinkDistance(d: Link, i: number) {
        ...
        if (d.source.Group === d.target.Group) {
            if (d.source.Metadata.Type === 'host') {
                for (let property in d.source.Edges)
                    distance += 2;
                return distance;
            }
        }
    ...
    InitFromSyncMessage(msg: Message) {
        ...
        for (let ID in this.graph.Edges)
            this.AddEdge(this.graph.Edges[ID]);

        if (store.state.currentNode) {
            let id = store.state.currentNode.ID;
    ...
    Rescale() {
        let trans = (d3.event as d3.ZoomEvent).translate;
        let scale = (d3.event as d3.ZoomEvent).scale;
    ...
    AddNode(node: GNode) {
        ...
        let data = lscache.get(node.Metadata.TID);
        if (data) {
            node.x = data.x;
            node.y = data.y;
            node.fixed = data.fixed;
        } else {
            let place = this.Hash(node.Host) % 100;
    ...
    AddEdge(edge: Edge) {
        ...
        if (edge.Parent.Metadata.Type === 'host') {
            ...
            let nodes = [edge.Parent, edge.Child];
            for (let n in nodes) {
                let node = nodes[n];
                for (i in node.Edges) {
                    e = node.Edges[i];
    ...
    NodeClass(d: GNode) {
        let clazz = 'node ' + d.Metadata.Type;
    ...
    UpdateGroups() {
        ...
        for (i in this.graph.Nodes) {
            ...
            let parent = this.ParentNodeForGroup(node);
        ...
        for (let i in this.nodes) {
            ...
            for (let j in node.Edges) {
                ...
                if ((edge.Parent === hostNode) || (edge.Child === hostNode)) {
                    ...
                    let found = false;
                    for (let n in child.Edges) {
                        let nEdge = edge.Child.Edges[n];
    ...
    Redraw() {
        ...
        if (typeof this.redrawTimeout === 'undefined')
            this.redrawTimeout = window.setTimeout(function () {
                for (let i in self.deferredActions) {
                    let action = self.deferredActions[i];
    ...
    redraw() {
        let _this = this;
        let linkEnter = this.link.data(this.links, function (d) { return d.source.ID + '-' + d.target.ID; });
        this.link = linkEnter;
        ...
        let nodeEnter = this.node.data(this.nodes, function (d) { return d.ID; })
            .attr('class', function (d) {
    ...
    ProcessGraphMessage(msg: Message) {
        ...
        switch (msg.Type) {
            ...
            case 'NodeUpdated':
                ...
                let redrawOn = ['Capture/ID', 'Status'],
            ...
            case 'EdgeAdded':
                let parent = this.graph.GetNode(msg.Obj.Parent);
                let child = this.graph.GetNode(msg.Obj.Child);
    ...
    SyncRequest(t: number) {
        ...
        let msg = { Namespace: 'Graph', Type: 'SyncRequest', Obj: obj };
        websocket.send(msg);
    }
}
